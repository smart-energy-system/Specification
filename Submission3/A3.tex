...
There are two possibilities for shifting demand which can be chosen by a user. The first one is used if the user wants to save time or has limited RAM. This variant allows only one shift per supplier. For example form 30\% of the load form 9 o'clock to 10 o'clock. This variant uses the constraint form \Cref{eq:numberOfShifts}. The second variant allows shifting form every hour to every other hour. This results depending on the input and the number of time steps in a very long run-time and a high RAM consumption.
